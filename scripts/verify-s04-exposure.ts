...
const scenarios: readonly ScenarioDefinition[] = [
  {
    ...
    expected: {
      ...
      absentWarnings: ['warning: Public exposure publishes authenticated proxy listeners on publicly routable IPs.'],
      routes: [
        {
  ...
  },
  {
    ...
    expected: {
      ...
      absentWarnings: ['info: Publish one DNS A record per route hostname and point it at the matching bind IP before expecting remote hostname access to work.'],
      routes: [
        {
          alias: 'sweden-gothenburg',
...
async function verifyScenario(scenario: ScenarioDefinition, options: VerificationOptions): Promise<string> {
  ...
  try {
    ...
    const manifestFile = await loadJsonFile<RuntimeBundleManifest>(manifestPath, 'runtime manifest');
    const manifest = options.simulateDrift && scenario.id === 'domain-private-network' ? tamperManifest(manifestFile) : manifestFile;
    ...
    assertNoSecretLeaks(`${scenario.id} runtime manifest`, JSON.stringify(manifestFile, null, 2), config);
...
async function verifyAmbiguousFailure(): Promise<string> {
  ...
  try {
    ...
    if (!result.stderr.includes('Mullgate exposure update failed.')) {
      throw new Error('ambiguous duplicate-bind-IP failure did not include the exposure failure header.');
    }

    if (!result.stderr.includes('code: AMBIGUOUS_SHARED_BIND_IP')) {
      throw new Error('ambiguous duplicate-bind-IP failure did not expose code AMBIGUOUS_SHARED_BIND_IP.');
    }

    if (!result.stderr.includes('Non-loopback multi-route exposure requires distinct bind IPs')) {
      throw new Error('ambiguous duplicate-bind-IP failure did not explain the duplicate bind-IP cause.');
    }

    const config = await loadConfig(store);

    if (config.setup.exposure.mode !== 'loopback') {
      throw new Error('ambiguous duplicate-bind-IP failure partially mutated the saved config instead of leaving loopback intact.');
    }
...
function assertSavedConfigMatchesScenario(config: MullgateConfig, scenario: ScenarioDefinition): void {
  ...
  if (config.setup.exposure.allowLan !== scenario.expected.allowLan) {
    throw new Error(`${scenario.id}: saved config allowLan drifted to ${String(config.setup.exposure.allowLan)}.`);
  }

  if (config.runtime.status.phase !== 'unvalidated') {
    throw new Error(`${scenario.id}: expected config runtime status to be unvalidated after CLI update, got ${config.runtime.status.phase}.`);
  }

  if (!config.runtime.status.message?.includes('Exposure settings changed')) {
    throw new Error(`${scenario.id}: config runtime status message no longer explains the restart-needed state.`);
  }

  const actualRoutes = config.routing.locations.map((route) => ({ alias: route.alias, hostname: route.hostname, bindIp: route.bindIp }));
  const expectedRoutes = scenario.expected.routes.map((route) => ({ alias: route.alias, hostname: route.hostname, bindIp: route.bindIp }));
...
function assertExposureSurface(scenario: ScenarioDefinition, output: string): void {
  assertContains(output, 'Mullgate exposure report', `${scenario.id}: exposure report header missing.`);
  assertContains(output, `mode: ${scenario.expected.mode}`, `${scenario.id}: exposure report mode line missing.`);
  assertContains(output, `base domain: ${scenario.expected.baseDomain ?? 'n/a'}`, `${scenario.id}: exposure report base domain line missing.`);
  assertContains(output, `allow lan: ${scenario.expected.allowLan ? 'yes' : 'no'}`, `${scenario.id}: exposure report allow-lan line missing.`);
  assertContains(output, 'runtime status: unvalidated', `${scenario.id}: exposure report runtime status line missing.`);
  assertContains(
    output,
  ...
  for (const guidanceLine of scenario.expected.guidance) {
    assertContains(output, `- ${guidanceLine}`, `${scenario.id}: exposure report guidance line missing: ${guidanceLine}`);
  }

  for (const warningLine of scenario.expected.warnings) {
    assertContains(output, `- ${warningLine}`, `${scenario.id}: exposure report warning line missing: ${warningLine}`);
  }

  for (const absent of scenario.expected.absentWarnings ?? []) {
    if (output.includes(absent)) {
      throw new Error(`${scenario.id}: exposure report unexpectedly included warning text: ${absent}`);
    }
  }

  for (const [index, route] of scenario.expected.routes.entries()) {
    assertContains(output, `${index + 1}. ${route.hostname} -> ${route.bindIp}`, `${scenario.id}: exposure route header missing for ${route.hostname}.`);
    assertContains(output, `   alias: ${route.alias}`, `${scenario.id}: alias line missing for ${route.hostname}.`);
    assertContains(output, `   ${route.dnsLine}`, `${scenario.id}: dns guidance line missing for ${route.hostname}.`);
    assertContains(
      output,
...
function assertHostsSurface(scenario: ScenarioDefinition, output: string): void {
  ...
  assertContains(output, 'copy/paste hosts block', `${scenario.id}: hosts report no longer prints the hosts block header.`);
  ...
  for (const [index, route] of scenario.expected.routes.entries()) {
    ...
    assertContains(output, `${route.bindIp} ${route.hostname}`, `${scenario.id}: hosts block line missing for ${route.hostname}.`);
  }
}

function assertManifestSurface(scenario: ScenarioDefinition, manifest: RuntimeBundleManifest): void {
  ...
  if (manifest.exposure.allowLan !== scenario.expected.allowLan) {
    throw new Error(`${scenario.id}: manifest exposure allowLan drifted to ${String(manifest.exposure.allowLan)}.`);
  }

  if (manifest.exposure.runtimeStatus.phase !== 'unvalidated') {
    throw new Error(`${scenario.id}: manifest runtime status drifted to ${manifest.exposure.runtimeStatus.phase}.`);
  }

  if (manifest.exposure.runtimeStatus.restartRequired !== scenario.expected.restartNeeded) {
    throw new Error(`${scenario.id}: manifest restart-needed drifted to ${String(manifest.exposure.runtimeStatus.restartRequired)}.`);
  }

  const actualRoutes = manifest.routes.map((route) => ({ alias: route.alias, hostname: route.hostname, bindIp: route.bindIp }));
  const expectedRoutes = scenario.expected.routes.map((route) => ({ alias: route.alias, hostname: route.hostname, bindIp: route.bindIp }));
  ...
  for (const warningLine of scenario.expected.warnings) {
    if (!manifest.exposure.warnings.some((warning) => `${warning.severity}: ${warning.message}` === warningLine)) {
      throw new Error(`${scenario.id}: manifest warning missing: ${warningLine}`);
    }
  }

  for (const absent of scenario.expected.absentWarnings ?? []) {
    if (manifest.exposure.warnings.some((warning) => `${warning.severity}: ${warning.message}` === absent)) {
  ...
  for (const [index, route] of scenario.expected.routes.entries()) {
    ...
    if (manifestRoute.alias !== route.alias || manifestRoute.hostname !== route.hostname || manifestRoute.bindIp !== route.bindIp) {
      throw new Error(
        [
    ...
    const expectedDnsRecord = route.dnsLine.startsWith('dns: ') ? route.dnsLine.replace(/^dns: /, '') : route.dnsLine;
    const actualDnsRecord = manifestRoute.dnsRecord ?? 'not required; use direct bind IP entrypoints';
...
function assertCliAndManifestAgree(scenario: ScenarioDefinition, output: string, manifest: RuntimeBundleManifest): void {
  for (const route of manifest.exposure.routes) {
    assertContains(output, `${route.index + 1}. ${route.hostname} -> ${route.bindIp}`, `${scenario.id}: CLI/manifest route header drifted for ${route.hostname}.`);
    assertContains(
      output,
...
function assertEndpointPresent(
  ...
  for (const [protocol, port] of [
    ...
    if (endpoint.redactedHostnameUrl !== `${protocol}://${REDACTED}:${REDACTED}@${hostname}:${port}`) {
      throw new Error(`${scenarioId}: manifest hostname endpoint drifted for ${hostname} (${protocol}).`);
    }

    if (endpoint.redactedBindUrl !== `${protocol}://${REDACTED}:${REDACTED}@${bindIp}:${port}`) {
      throw new Error(`${scenarioId}: manifest bind-IP endpoint drifted for ${hostname} (${protocol}).`);
    }
  }
}
