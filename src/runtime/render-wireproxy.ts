...
const WIREPROXY_IMAGE = 'ghcr.io/windtf/wireproxy:latest';
...
export function planWireproxyArtifacts(options: RenderWireproxyOptions): RenderWireproxyResult {
  ...
  const dockerCompose = buildDockerCompose(options.config, options.paths);
  ...
  return {
    ...
    artifactPaths: {
      ...
      dockerComposePath: options.paths.dockerComposePath,
...
export async function renderWireproxyArtifacts(options: RenderWireproxyOptions): Promise<RenderWireproxyResult> {
  ...
  try {
    ...
    await writeFileAtomic(planned.artifactPaths.dockerComposePath, planned.dockerCompose, 0o600);
...
function buildDockerCompose(config: MullgateConfig, paths: MullgatePaths): string {
  const composePath = paths.wireproxyConfigFile;
  const ports = [
  ...
  return [
    ...
    '    command:',
    `      - --config`,
    `      - /etc/wireproxy/${path.basename(composePath)}`,
    '    volumes:',
    `      - ${composePath}:/etc/wireproxy/${path.basename(composePath)}:ro`,
    '    ports:',
    ...ports,
  ].join('\n') + '\n';
}
