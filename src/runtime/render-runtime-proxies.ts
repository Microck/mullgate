...
export type RenderedRouteProxyRoute = RouteProxyIdentity & {
  ...
  readonly exitSocksInternalIp: string | null;
...
export type RenderRuntimeProxySuccess = {
  ...
  readonly entryRelay: MullvadRelay | null;
...
export function planRuntimeProxyArtifacts(
  ...
  const usesTailscaleExit = getExitSource(options.config) === 'tailscale-exit';
  ...
  const entryRelayResult = usesTailscaleExit
    ? ({
        ok: true,
        value: {
          relay: null,
  ...
  const routes = inlineSelectorEnabled
    ...
    : options.config.routing.locations.map((route, index) => {
        ...
        if (
          ...
          !exit.socksHostname ||
          !exit.socksPort ||
          (usesTailscaleExit && !exit.socksInternalIp)
        ) {
          return {
            ok: false as const,
            route,
        ...
        return {
          ...
          value: {
            ...
            exitSocksInternalIp: exit.socksInternalIp ?? null,
            entryParent: usesTailscaleExit
              ? null
              : {
  ...
  return {
    ...
    entryWireproxyConfig: entryRelayResult.value.relay
      ? buildEntryWireproxyConfig({
          config: options.config,
          relay: entryRelayResult.value.relay,
          generatedAt: checkedAt,
        })
      : buildTailscaleEntryPlaceholderConfig({ generatedAt: checkedAt }),
...
function buildTailscaleEntryPlaceholderConfig(input: { readonly generatedAt: string }): string {
  return `${[
    '# Generated by Mullgate. Derived artifact; edit canonical config instead.',
    `# Generated at ${input.generatedAt}`,
...
function buildRouteProxyConfig(input: {
  ...
  const authMode =
    input.config.setup.access.mode === 'inline-selector' && input.config.setup.auth.password === ''
      ? 'auth useronly'
      : 'auth strong';
  const lines = [
    '# Generated by Mullgate. Derived artifact; edit canonical config instead.',
  ...
  if (input.config.setup.access.mode === 'inline-selector') {
    ...
    for (const selector of input.inlineSelectors) {
      const parentHost = selector.exitSocksInternalIp ?? selector.exitSocksHostname;
      lines.push(
        `# Selector ${selector.selector} -> ${selector.exitRelayHostname}`,
        `allow ${selector.selector}`,
        ...(getExitSource(input.config) === 'tailscale-exit'
          ? []
          : [`parent 1000 socks5+ 127.0.0.1 ${ENTRY_WIREPROXY_SOCKS_PORT}`]),
        `parent 1000 socks5+ ${parentHost} ${selector.exitSocksPort}`,
      );
    }
  ...
  for (const route of input.routes) {
    ...
    const parentHost = route.exitSocksInternalIp ?? route.exitSocksHostname;
    const entryParentLines = route.entryParent
    ...
    lines.push(
      ...
      ...entryParentLines,
      `parent 1000 socks5+ ${parentHost} ${route.exitSocksPort}`,
      ...
      return [
        {
          ...
          exitSocksInternalIp: relay.socksInternalIp ?? null,
