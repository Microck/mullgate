...
- missing `tailscale-exit` credentials or route internal SOCKS IPs when that exit source is enabled

Use `mullgate proxy status` and `mullgate proxy doctor` to identify which layer failed.

### `tailscale-exit` starts but routes are missing internal SOCKS IPs

Likely cause:

- the route artifacts were rendered before Mullgate resolved the `10.124.x.x` Mullvad SOCKS endpoints
- relay metadata or DNS resolution was unavailable during setup

What to do:

1. confirm `MULLGATE_TAILSCALE_TAILNET`, `MULLGATE_TAILSCALE_AUTH_KEY`, and `MULLGATE_TAILSCALE_PINNED_EXIT_NODE` are set for setup
2. run `mullgate proxy validate --refresh`
3. run `mullgate proxy doctor`
4. if the environment is new, run `pnpm verify:tailscale-feasibility` before relying on it
...
2. inspect hostname guidance or selector examples in that same report
...
- [Tailscale Exit Source](/docs/guides/tailscale-exit-source)
