---
title: Inline Selector Reference
description: Supported selector forms, URL syntax, and practical caveats for inline-selector access.
---
...
- a bare city slug such as `gothenburg` or `toronto` is only usable if it is unique in the catalog
