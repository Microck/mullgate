...
Use `mullgate export` when you want a ready-to-paste inventory of authenticated route URLs instead of assembling client config by hand.
...
mullgate export --stdout --protocol socks5 --region europe
```
...
If HTTPS proxy support is configured with a cert, key, and HTTPS port, the same route inventory appears in `mullgate exposure`, `start`, `status`, and `runtime-manifest.json`.
