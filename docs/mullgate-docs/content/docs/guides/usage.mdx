...
Use `mullgate config export` when you want a ready-to-paste inventory of authenticated route URLs instead of assembling client config by hand.
...
mullgate config export --stdout --protocol socks5 --region europe
```

- `--dry-run` prints a secret-safe preview with credentials redacted and does not write a file
