...
See [Inline Selector Reference](/docs/guides/inline-selector-selectors) for the supported selector families and caveats.
...
## Can Mullgate use Tailscale's Mullvad add-on instead?

Yes. Use the opt-in `tailscale-exit` source when your tailnet has the Mullvad add-on enabled and you want one Tailscale sidecar to carry route-proxy traffic to Mullvad internal SOCKS endpoints. See [Tailscale Exit Source](/docs/guides/tailscale-exit-source).
...
4. [Tailscale Exit Source](/docs/guides/tailscale-exit-source), if your tailnet uses the Mullvad add-on
5. [Current Runtime Model](/docs/architecture/current-runtime)
6. [Multi-exit Architecture](/docs/architecture/multi-exit)
