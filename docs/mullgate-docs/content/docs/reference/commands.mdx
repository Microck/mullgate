...
`mullgate config export` is the proxy-list generation surface. It supports ordered repeated selectors such as `--country se --count 3 --region europe --count 2`, guided `proxies.txt` creation via `--guided`, preview mode via `--dry-run`, stdout delivery via `--stdout`, and safe file overwrite behavior with `--force`.
