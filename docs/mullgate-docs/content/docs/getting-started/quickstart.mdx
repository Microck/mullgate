...
4. start the runtime with `mullgate start`
5. verify behavior with `mullgate status` and `mullgate doctor`
6. access routes by hostname or direct bind IP
...
That gives you a guided `proxies.txt` flow with real country and region pick-lists instead of hand-editing URLs or typing country codes from memory.
...
6. [Release Runbook](/docs/operations/release-runbook)
