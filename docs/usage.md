...
`mullgate setup` prompts for the exit source first, then collects either the default Mullvad WireGuard inputs or the `tailscale-exit` inputs. `mullgate setup --non-interactive` fails instead of prompting when required inputs are missing. You can supply values with flags, environment variables, or a mix of both.

### Required for the default setup run
...
### Required for `tailscale-exit`

Choose `tailscale-exit` in the guided setup, or set `MULLGATE_EXIT_SOURCE=tailscale-exit`, when you want one Tailscale sidecar pinned to a Mullvad exit instead of the default Mullvad WireGuard entry device.

| Purpose | Flag | Environment variable |
| --- | --- | --- |
| Exit source | `--exit-source tailscale-exit` | `MULLGATE_EXIT_SOURCE` |
| Tailscale tailnet | `--tailscale-tailnet <name>` | `MULLGATE_TAILSCALE_TAILNET` |
| Tailscale auth key | `--tailscale-auth-key <key>` | `MULLGATE_TAILSCALE_AUTH_KEY` |
| Pinned Mullvad exit node | `--tailscale-pinned-exit-node <exit-node>` | `MULLGATE_TAILSCALE_PINNED_EXIT_NODE` |
| Proxy username | `--username <name>` | `MULLGATE_PROXY_USERNAME` |
| One or more routed locations | `--location <alias>` (repeatable) | `MULLGATE_LOCATION` or `MULLGATE_LOCATIONS` |
...
### Example: Tailscale-backed exit source

Use this when your tailnet has the Tailscale Mullvad add-on enabled and you want one sidecar device to reach multiple Mullvad internal SOCKS exits:
...
The pinned exit node is only the carrier path into Mullvad's internal network. Each route still resolves and connects to its own `10.124.x.x:1080` Mullvad SOCKS endpoint.
...
## Tailscale feasibility verifier

Use `pnpm verify:tailscale-feasibility` before depending on `tailscale-exit` in a new environment. The verifier starts a temporary Tailscale sidecar container, pins it to one Mullvad exit node, resolves internal Mullvad SOCKS IPs through public DNS, and probes three direct `10.124.x.x:1080` SOCKS endpoints concurrently.

Required live inputs:

| Purpose | Environment variable |
| --- | --- |
| Tailscale tailnet | `MULLGATE_TAILSCALE_TAILNET` |
| Tailscale auth key | `MULLGATE_TAILSCALE_AUTH_KEY` |
| Pinned Mullvad exit node | `MULLGATE_TAILSCALE_PINNED_EXIT_NODE` |

For the pinned exit node, prefer the Tailscale IP from `tailscale exit-node list`.
...
Fixture replay is available for deterministic review:
