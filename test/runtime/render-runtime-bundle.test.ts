  ...
  it('renders a multi-route compose bundle, HAProxy router config, and manifest from canonical settings', async () => {
    ...
    expect(`\n${compose}`).toMatchInlineSnapshot(`
      ...
      services:
        entry-tunnel:
          ...
          volumes:
            - /tmp/mullgate-home/state/mullgate/runtime/entry-wireproxy.conf:/etc/wireproxy/wireproxy.conf:ro
        route-proxy:
          ...
          volumes:
            - /tmp/mullgate-home/state/mullgate/runtime/route-proxy.cfg:/etc/3proxy/3proxy.cfg:ro
        routing-layer:
    ...
    expect(`\n${httpsSidecarConfig}`).toMatchInlineSnapshot(`
      ...
      frontend https_proxy
        ...
        use_backend route-se-got-wg-101 if route_se_got_wg_101_https
        acl route_at_vie_wg_001_https dst 127.0.0.2
        use_backend route-at-vie-wg-001 if route_at_vie_wg_001_https
        default_backend route-se-got-wg-101

      backend route-se-got-wg-101
        server se-got-wg-101 127.0.0.1:8080 check

      backend route-at-vie-wg-001
    ...
    expect(parsedManifest.routes.map((route) => route.services.backends)).toEqual([
      { https: 'route-se-got-wg-101' },
      { https: 'route-at-vie-wg-001' },
    ]);
  });
