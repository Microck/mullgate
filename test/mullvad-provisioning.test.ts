  ...
  it('provisions a WireGuard device, renders wireproxy artifacts, and validates through the Docker fallback', async () => {
    ...
    await withJsonServer(
      ...
      async (baseUrl) => {
        ...
        const dockerCompose = await readFile(renderResult.artifactPaths.dockerComposePath, 'utf8');
        ...
        expect(dockerCompose).toContain('ghcr.io/windtf/wireproxy:latest');
...
"
`);
        expect('\n' + normalizePathInput(dockerCompose, env.HOME!)).toMatchInlineSnapshot(`
"\n# Generated by Mullgate. Derived artifact; edit canonical config instead.
services:
  wireproxy:
